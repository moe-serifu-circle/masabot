...
class CustomRoleModule(BotBehaviorModule):
	...
	def __init__(self, resource_root: str):
		...
		help_text += " one.\n\n"
		help_text += "__Command Forms__\n"
		help_text += "`!role` - Create or change the custom role that you have. If you already have one, I will ask"
		help_text += " you for a color and then update your role to match. If you don't have one, I'll ask you for a color"
		help_text += " and a name to call it!\n"
		help_text += "`!role [COLOR]` - Create or change the custom role that you have, setting the color to the one that"
		help_text += " you give. If you don't yet have a role, I'll ask you for a name for it. COLOR needs to be a hex"
		help_text += " color code such as `#ff23ac` or `2FE192`.\n"
		help_text += "`!role [COLOR] [NAME]` - Create or change the custom role that you have so that it has the given"
		help_text += " color and name. If you don't have one yet, one is created! COLOR needs to be a hex color code"
		help_text += " such as `#ff23ac` or `2FE192`, and NAME is what you want to change the name of the role to.\n"
		help_text += "`!role-set USER [COLOR] [NAME]` - Op-only command to set the assigned custom vanity role for"
		help_text += " USER to the given COLOR and NAME. Syntax is same as `!role` command except that giving the USER"
		help_text += " to change is required, and if NAME is an existing role, it will be assigned as the custom role"
		help_text += " instead of a new one being created.\n"
		help_text += "`!role-info` - Shows what your current custom color vanity role is set to.\n"
		help_text += "`!role-info [USER]` - Shows what another server member's custom color vanity role is set to.\n"
		help_text += "`!role-remove` - Remove your custom color vanity role.\n"
		help_text += "`!role-remove [USER]` - Op-only version of the role-remove command that removes someone else's"
		help_text += " custom color vanity role.\n\n"
